...
class TestUserLogin(unittest.TestCase):
    @classmethod
    def setUpClass(cls):   # 初始化操作，整个测试类只执行一次
        cls.data_list = excel_to_list(data_path, "TestUserLogin")  # 读取TestUserReg工作簿的所有数据
    ...
    def test_user_login_normal(self):
        ...
        res = requests.post(url=url, data=json.loads(data))  # 表单请求，数据转为字典格式,文本格式
        # res=requests.post(url=url,json=data) 使用字符串格式
